...
class KaptIncrementalWithAggregatingApt : KaptIncrementalIT() {
    ...
    fun testIncrementalAggregatingChangesWithJdk9() {
        ...
        Assume.assumeTrue("JDK 9 isn't available", javaHome.isDirectory)
        val options = defaultBuildOptions().copy(javaHome = javaHome)
        doIncrementalAggregatingChanges(options)
    }

    fun doIncrementalAggregatingChanges(buildOptions: BuildOptions = defaultBuildOptions()) {
        val project = Project(
            "kaptIncrementalAggregatingProcessorProject",
            GradleVersionRequired.None
        ).apply {
            setupIncrementalAptProject("ISOLATING" to IncrementalIsolatingProcessor::class.java, "AGGREGATING" to IncrementalAggregatingProcessor::class.java)
        }
